...
function Sign({ children }: { children: ReactNode }) {
  return <div className="orange-gradient inline-flex items-center gap-2 rounded-lg border-2 border-white/70 p-1.5 pr-4 shadow-lg"><span className="flex h-9 w-9 flex-col items-center justify-center rounded border-2 border-[#d65a27] bg-white font-exo text-[9px] font-bold leading-none text-[#153283]">KM<span className="text-lg">20</span></span><h2 className="font-exo text-lg font-bold uppercase leading-none text-white sm:text-2xl">{children}</h2></div>;
}

function FlowColumn({ title, steps }: { title: string; steps: string[] }) {
  return <div className="rounded-xl bg-[#080f65]/75 p-3 shadow-inner sm:p-4"><h3 className="mb-3 font-exo text-base font-bold uppercase leading-[.9] text-white sm:text-lg">{title.slice(0, 4)}<br /><span className="text-[#f0782d]">{title.slice(4)}</span></h3><ol className="flex flex-col items-center gap-1 text-center font-exo text-[11px] font-semibold text-white sm:text-xs">{steps.map((step, index) => <li key={step} className="flex w-full flex-col items-center">{index > 0 && <span className="mb-1 text-lg leading-none text-[#f0782d]">⌄</span>}<span><b className="text-[#f0782d]">{index + 1}</b> {step}</span></li>)}</ol></div>;
}
    ...
    <header className="absolute inset-0 z-20 px-4 py-5 sm:px-8 lg:px-10 lg:py-8">
      <div className="mx-auto flex h-full max-w-[1280px] flex-col">
        <div className="flex items-center justify-between gap-3">
          <a href="#inicio" className="flex h-auto w-auto items-center justify-center rounded-full bg-white px-1 py-0.5 shadow-lg transition-transform hover:scale-105" aria-label="LUTSA Transportes, ir a inicio"><img src="/seccion01/svg/btn_lutsa_superior.svg" alt="LUTSA Transportes" className="h-6 w-auto object-contain sm:h-8" /></a>
        ...
        <div className="mt-5 grid gap-6 lg:grid-cols-[1.05fr_1.35fr_1fr_1fr] lg:gap-7">
          <div className="min-w-0"><Sign>Infraestructura<br /><span className="text-[#10227f]">y ubicación</span></Sign><ul className="mt-4 space-y-3 font-exo text-xs font-bold uppercase leading-[1.05] text-white sm:text-sm">{infrastructure.map((point, index) => <li key={index}>{point}</li>)}</ul></div>
          ...
          <div className="grid min-w-0 gap-6 sm:grid-cols-2 lg:contents"><div><h2 className="font-exo text-2xl font-bold uppercase text-[#f0782d] sm:text-3xl">Capacidades</h2><ul className="mt-3 space-y-2 font-exo text-xs font-semibold text-white sm:text-sm">{capabilities.map((item) => <li key={item} className="border-b border-[#f0782d] pb-1.5">{item}</li>)}</ul></div><div><h2 className="font-exo text-2xl font-bold uppercase text-white sm:text-3xl">Tipo de remolques</h2><ul className="mt-3 space-y-2 font-exo text-xs font-bold text-white">{trailers.map((item) => <li key={item} className="orange-gradient rounded-xl px-3 py-3 text-center transition-transform hover:-translate-y-1 hover:shadow-glow">{item}</li>)}</ul></div></div>
        </div>
      </div>
    </header>
  );
