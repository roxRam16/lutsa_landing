    ...
    <header className="absolute inset-0 z-20 px-4 py-5 sm:px-8 lg:px-10 lg:py-8">
      <div className="mx-auto flex h-full max-w-[1280px] flex-col">
        ...
        <div className="mt-5 grid gap-6 lg:grid-cols-[1.05fr_1.35fr_1fr_1fr] lg:gap-7">
          ...
          <div className="grid min-w-0 gap-6 sm:grid-cols-2 lg:contents"><div className="min-w-0"><Sign>Capacidades</Sign><ul className="mt-6 space-y-2 font-exo text-[10px] font-semibold text-white">{capabilities.map((item) => <li key={item} className="border-b border-[#f0782d] pb-1.5">{item}</li>)}</ul></div><div className="min-w-0"><Sign>Tipo de remolques</Sign><ul className="mt-6 space-y-2 font-exo text-[10px] font-bold text-white">{trailers.map((item) => <li key={item} className="orange-gradient rounded-xl px-3 py-3 text-center transition-transform hover:-translate-y-1 hover:shadow-glow">{item}</li>)}</ul></div></div>
        </div>
      </div>
    </header>
  );
