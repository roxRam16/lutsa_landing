...
function Sign({ children }: { children: ReactNode }) {
  return <div className="orange-gradient inline-flex items-center gap-2 rounded-lg border-2 border-white/70 p-1.5 pr-4 shadow-lg"><span className="flex h-9 w-9 flex-col items-center justify-center rounded border-2 border-[#d65a27] bg-white font-exo text-[9px] font-bold leading-none text-[#153283]">KM<span className="text-lg">20</span></span><h2 className="font-exo text-[12px] font-bold uppercase leading-none text-white">{children}</h2></div>;
}

function FlowColumn({ title, steps }: { title: string; steps: string[] }) {
  return <div className="rounded-xl bg-[#080f65]/75 p-3 shadow-inner sm:p-4"><h3 className="mb-3 font-exo text-[12px] font-bold uppercase leading-[.9] text-white">{title.slice(0, 4)}<br /><span className="text-[#f0782d]">{title.slice(4)}</span></h3><ol className="flex flex-col items-center gap-1 text-center font-exo text-[12px] font-semibold text-white">{steps.map((step, index) => <li key={step} className="flex w-full flex-col items-center">{index > 0 && <span className="mb-1 text-lg leading-none text-[#f0782d]">⌄</span>}<span><b className="text-[#f0782d]">{index + 1}</b> {step}</span></li>)}</ol></div>;
}
    ...
    <header className="absolute inset-0 z-20 px-4 py-5 sm:px-8 lg:px-10 lg:py-8">
      <div className="mx-auto flex h-full max-w-[1280px] flex-col">
        ...
        <div className="mt-5 grid gap-6 lg:grid-cols-[1.05fr_1.35fr_1fr_1fr] lg:gap-7">
          <div className="min-w-0"><Sign>Infraestructura<br /><span className="text-[#10227f]">y ubicación</span></Sign><ul className="mt-4 space-y-3 font-exo text-[12px] font-bold uppercase leading-[1.05] text-white">{infrastructure.map((point, index) => <li key={index}>{point}</li>)}</ul></div>
          ...
          <div className="grid min-w-0 gap-6 sm:grid-cols-2 lg:contents"><div><h2 className="font-exo text-[12px] font-bold uppercase text-[#f0782d]">Capacidades</h2><ul className="mt-3 space-y-2 font-exo text-[12px] font-semibold text-white">{capabilities.map((item) => <li key={item} className="border-b border-[#f0782d] pb-1.5">{item}</li>)}</ul></div><div><h2 className="font-exo text-[12px] font-bold uppercase text-white">Tipo de remolques</h2><ul className="mt-3 space-y-2 font-exo text-[12px] font-bold text-white">{trailers.map((item) => <li key={item} className="orange-gradient rounded-xl px-3 py-3 text-center transition-transform hover:-translate-y-1 hover:shadow-glow">{item}</li>)}</ul></div></div>
        </div>
      </div>
    </header>
  );
