...
function Sign({ children }: { children: ReactNode }) {
  return (
    <div className="orange-gradient inline-flex items-center gap-2 rounded-lg border-2 border-white/70 p-1.5 pr-4 shadow-lg">
      <span className="flex h-8 w-8 flex-col items-center justify-center rounded border-2 border-[#d65a27] bg-white font-exo text-[8px] font-bold leading-none text-[#153283]">KM<span className="text-sm">20</span></span>
      <h2 className="font-exo text-[10px] font-bold uppercase leading-none text-white">{children}</h2>
    </div>
  );
}
    ...
    <section id="servicios" className="relative overflow-hidden">
      ...
      <div className="relative z-10 px-4 pt-20 pb-10 sm:px-8 lg:px-10 lg:pt-24 lg:pb-12">
        <div className="mx-auto max-w-[1280px]">
          ...
          <div className="mt-4 -mx-4 bg-gradient-to-r from-[#141c80]/40 via-[#132085]/35 to-[#11166d]/40 px-4 py-4 sm:-mx-8 sm:px-8 sm:py-5 lg:-mx-10 lg:px-10">
          ...
          <div className="mt-5 grid gap-6 lg:grid-cols-[1.05fr_1.35fr_1fr_1fr] lg:gap-7">
            <div className="min-w-0">
              ...
              <ul className="mt-4 space-y-3 font-exo text-[10px] font-bold uppercase leading-[1.05] text-white">
                {infrastructure.map((point, index) => <li key={index}>{point}</li>)}
              </ul>
            </div>
            <div className="min-w-0">
              <Sign>Operación integral<br /><span className="text-[#10227f]">flujos operativos</span></Sign>
            ...
            <div className="grid min-w-0 gap-6 sm:grid-cols-2 lg:contents">
              <div className="min-w-0 lg:pt-[108px]">
                <h2 className="font-exo text-[10px] font-bold uppercase text-[#f0782d]">Capacidades</h2>
                <ul className="mt-3 space-y-2 font-exo text-[10px] font-semibold text-white">
                  {capabilities.map((item) => <li key={item} className="border-b border-[#f0782d] pb-1.5">{item}</li>)}
                </ul>
              </div>
              <div className="min-w-0">
                <h2 className="font-exo text-[10px] font-bold uppercase text-white">Tipo de remolques</h2>
                <ul className="mt-3 space-y-2 font-exo text-[10px] font-bold text-white">
                  {trailers.map((item) => <li key={item} className="orange-gradient rounded-xl px-3 py-3 text-center transition-transform hover:-translate-y-1 hover:shadow-glow">{item}</li>)}
                </ul>
              </div>
            </div>
          </div>
